...
public class Patients_Details_Actions extends BaseClass {
	...
	public void The_Left_Side_Menu_for_Phlebotomist_Dashboard() throws IOException {
		...
		if (patients_details_locators.Patient_Record_Link().isEnabled()) {
			...
			String actualPateintRecordUrl = PropertiesReader.getPropertyValue(WebCommonPath.testDatafile,
					"actualPateintRecordUrl");
		...
		if (patients_details_locators.Resource_Link().isEnabled()) {
			...
			String actualResourceUrl = PropertiesReader.getPropertyValue(WebCommonPath.testDatafile,
					"actualResourceUrl");
	...
	public void Valid_Patients_Details_Case7() {
		...
		/*
		 * WebWait.visibilityOfElement(driver, patients_details_locators.getBookingId(),
	...
	public void Valid_Patients_Details_Case12(String BookingId, String fname, String lastname) {
		WebWait.elementToBeClickable(driver, patients_details_locators.Enable_Later(), Duration.ofSeconds(30));
		WebButton.clickButton(patients_details_locators.Enable_Later());

		WebWait.visibilityOfElement(driver, patients_details_locators.getBookingId(), Duration.ofSeconds(30));
		WebTextBox.sendInput(patients_details_locators.getBookingId(), BookingId);

		WebWait.visibilityOfElement(driver, patients_details_locators.getFirstName(), Duration.ofSeconds(30));
		WebTextBox.sendInput(patients_details_locators.getFirstName(), fname);

		WebWait.visibilityOfElement(driver, patients_details_locators.getLastName(), Duration.ofSeconds(30));
		WebTextBox.sendInput(patients_details_locators.getLastName(), lastname);
	...
	public void PatientDetailsDisplayCorrectly() throws InterruptedException {
		...
		if (patients_details_locators.getpatient_Name().isDisplayed()) {
			String getpatient_Name_AutoPopulated = patients_details_locators.getpatient_Name_AutoPopulated()
					.getAttribute("value");
		...
		if (patients_details_locators.getDOB().isDisplayed()) {
			String getpatient_DOB_AutoPopulated = patients_details_locators.getDOB_AutoPopulated()
					.getAttribute("value");
		...
		if (patients_details_locators.getpatient_Email().isDisplayed()) {
			String getpatient_Email_AutoPopulated = patients_details_locators.getpatient_Email_AutoPopulated()
					.getAttribute("value");
		...
		if (patients_details_locators.getpatient_Phone().isDisplayed()) {
			String getpatient_Phone_AutoPopulated = patients_details_locators.getpatient_Phone_AutoPopulated()
					.getAttribute("value");
		...
		if (patients_details_locators.getpatient_Test_Type().isDisplayed()) {
			String getpatient_Test_Type_AutoPopulated = patients_details_locators.getpatient_Test_Type_AutoPopulated()
					.getAttribute("value");
		...
		if (patients_details_locators.getpatient_Vessel_Type().isDisplayed()) {
			String getpatient_Vessel_Type_AutoPopulated = patients_details_locators
					.getpatient_Vessel_Type_AutoPopulated().getAttribute("value");
		...
		if (patients_details_locators.getpatient_Repeated_Test().isDisplayed()) {
			String getpatient_Repeated_Test_AutoPopulated = patients_details_locators
					.getpatient_Repeated_Test_AutoPopulated().getAttribute("value");
	...
	public void functionality_Of_Back_Button() throws IOException, InterruptedException {
		...
		String getPateintRecordUrl = driver.getCurrentUrl();
		String actualPateintRecordUrl = PropertiesReader.getPropertyValue(WebCommonPath.testDatafile,
				"actualPateintRecordDetailsUrl");
		Assert.assertEquals(getPateintRecordUrl, actualPateintRecordUrl);
		Thread.sleep(3000);
		WebButton.clickButton(patients_details_locators.getRegistered_Button());
		String getConfirmationPageUrl = driver.getCurrentUrl();
		String actualConfirmationPageUrl = PropertiesReader.getPropertyValue(WebCommonPath.testDatafile,
				"actualTestConfirmationUrl");
	...
	public void functionality_Test_Name_Populated() throws IOException, InterruptedException {
		...
		if (patients_details_locators.gettest_type_autopopulated().isDisplayed()) {
			String get_Test_Type_AutoPopulated = patients_details_locators.gettest_type_autopopulated()
					.getAttribute("value");
	...
	public void functionality_PGPL_Barcode_And_Data_Populated() throws IOException, InterruptedException {
		...
		// WebButton.JsclickButton(patients_details_locators.getconfirmation_table_register_btn(),
		// driver);
	...
	public void functionality_Double_Lavender_Barcode_Success_Toaster_Msg() throws IOException, InterruptedException {
		 ...
		 * WebTextBox.sendInput(patients_details_locators.getbarcode_date(),"04042025");
		 * Thread.sleep(2000);
		 * WebButton.JsclickButton(patients_details_locators.getsave_and_continue_btn(),
		 * driver); //Thread.sleep(2000); // Need to add one more step of assertion
		 * success msg, not added due to test data
		 */
	}

	// 59,60,61 Test_Data_Double_Barcode_with save and continue
	// _success_msg----------
	...
	public void functionality_Reject_Kit_But_Not_Effect_To_Customer() throws Exception {
		...
		WebTextBox.sendInput(patients_details_locators.getprimary_barcode(), "PGZ-HXM-5500"); // Barcode B for
																								// completion process
		WebButton.clickButton(patients_details_locators.getbarcode_image());
		Thread.sleep(3000);
	...
	public void register_Patient(String primary_barcode) throws InterruptedException {
		WebWait.elementToBeClickable(driver, patients_details_locators.getRegistered_Button(), Duration.ofSeconds(20));
		WebButton.clickButton(patients_details_locators.getRegistered_Button());
		Store_Location_Search_Functionality();
		WebButton.clickButton(patients_details_locators.getsave_and_continue_btn());
		Thread.sleep(2000);
		WebTextBox.sendInput(patients_details_locators.getprimary_barcode(), primary_barcode);
		// patients_details_locators.getprimary_barcode().sendKeys(Keys.TAB);
		
		WebDropDown.searchAndSelect(driver, patients_details_locators.getbarcode_date(), "19/05/2025");
		WebTextBox.sendInput(patients_details_locators.getbarcode_time(),"18:00");
		//Thread.sleep(5000);
		
		
		
		WebDropDown.selectByText(patients_details_locators.getbarcode_No_Of_Attempts(), "1");
		WebTextBox.sendInput(patients_details_locators.getbarcode_Notes(), "Test");
